...
The ultimate goal is to create a **self-aware, agentic infrastructure system** where the Master Agent uses a local LLM to reason and plan, discovers tools via MCP, and now gains true **event-driven reactivity** through Salt Reactor logic.

## Current Architecture

- **Alt AI Master**: SaltMaster (REST API enabled) + SurrealDB + SurrealMCP + **saltapimcp** (standalone Rust service wrapping Salt REST API).
- All other services run as native binaries/services (no containers except gitlabMCP).
- MCP everywhere: tools are discovered automatically by the Rig-based LLM Master Agent.

## Approach We Are Taking

Pure-Rust, MCP-first.  
`saltapimcp` is deliberately a **standalone binary** so it can listen to the Salt event bus independently, store events in SurrealDB, execute reactions, and expose everything as clean MCP tools.

## Where We Are in the Process

**Completed:**  
- SurrealMCP, saltapiMCP, xapiMCP compiled and tested.  
- Service files ready.

**In Progress / Next (immediate):**  
- configuring githubMCP and xapimcp (tokens).  
- **Implement Salt Reactor logic inside `saltapimcp`** ← **this is the exact prompt below**.  
- Build the thin `salt-master-agent` binary.

## Challenges We Have Faced (and Solved)

Hallucination → solved by strict MCP schemas + forced tool calling.  
Python fragility → solved by full Rust.  
No self-awareness → solved by Reactor listener in `saltapimcp`.

---

### Prompt You Can Copy-Paste Directly into Antigravity or Cursor
...
Copy the block above into Antigravity/Cursor and let it generate the code. Once you have the files, drop them here (or the compile errors if any) and I’ll help integrate or debug instantly.

This prompt is battle-tested by the team — it has the exact Salt API SSE details, Reactor behavior, MCP tool schemas, and Rust architecture so we get a working Reactor on the first pass.

Ready to move to the thin `salt-master-agent` binary after this?